...
class TestRosbag2Reader:
    ...
    def test_explicit_sensor2ego_translation_only(self, bag_with_pointclouds: Path) -> None:
        ...
        with Rosbag2Reader(str(bag_with_pointclouds), topic_mapping=mapping) as reader:
            ...
            np.testing.assert_array_almost_equal(pc.points[:3, 0], [10.0, 21.0, 32.0])

    def test_explicit_sensor2ego_overrides_frame_id(self, bag_with_pointclouds: Path) -> None:
        ...
        with Rosbag2Reader(str(bag_with_pointclouds), topic_mapping=mapping) as reader:
            pc = reader.get_pointcloud("LIDAR_TOP", 1_704_067_200_000_000)
            np.testing.assert_array_almost_equal(pc.points[:3, 0], [1.0, 3.0, 5.0])

    def test_explicit_sensor2ego_quaternion_rotation(self, bag_with_pointclouds: Path) -> None:
        ...
        with Rosbag2Reader(str(bag_with_pointclouds), topic_mapping=mapping) as reader:
            ...
            np.testing.assert_array_almost_equal(pc.points[:3, 0], [-1.0, 0.0, 2.0])
