    ...
    def render_map(self, filepath: str) -> None:
        ...
        for relation in parser.relations.values():
            ...
            if left_bound and right_bound:
                ...
                if subtype == "road":
                    color = colors["lanelet_road"]
                    element_type = "road"
                elif subtype == "crosswalk":
                    color = colors["lanelet_crosswalk"]
                    element_type = "crosswalk"
                else:
                    color = colors["lanelet_shoulder"]
                    element_type = "shoulder"

                entity_path = format_entity(
                    self.map_entity, f"lanelet/{element_type}/{relation.id}"
                )
                rr.log(
                    entity_path,
                    rr.Mesh3D(
            ...
            for member in relation.members:
                if member.type == "way" and member.role in ["ref_line", "refers"]:
                    ...
                    for i, center in enumerate(coords):
                        entity_path = format_entity(
                            self.map_entity, f"traffic_elements/{element_type}/{relation.id}_{i}"
                        )
